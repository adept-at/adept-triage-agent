...
class PipelineCoordinator {
    ...
    async execute() {
        ...
        if (autoFixResult?.success && autoFixResult.validationStatus === 'passed' && skillStore) {
            const latestSkills = skillStore.findRelevant({
                framework: errorData.framework || 'unknown',
                spec: errorData.fileName,
                errorMessage: errorData.message,
                limit: 1,
            ...
            if (latestSkills.length > 0) {
                await skillStore.recordClassificationOutcome(latestSkills[0].id, 'correct').catch(() => { });
            }
        }
