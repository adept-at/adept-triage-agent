...
> **Last aligned with code:** v1.52.9. Cross-reference with `docs/ARCHITECTURE.md` for the authoritative validation-path gates.
...
**Consumer repos as of v1.52.9:** `adept-at/wdio-9-bidi-mux3`, `adept-at/lib-cypress-canary`, `adept-at/lib-wdio-8-e2e-ts`, `adept-at/lib-wdio-8-multi-remote`, `adept-at/learn-webapp`. See `seeds/DEPLOYED.md` for which have `.adept-triage/context.md` deployed and which use the bundled path.
