...
export class OpenAIClient {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (attempt === this.maxRetries) {
          // Try GPT-3.5 as fallback (without screenshots)
  ...
  private async fallbackToGPT35(errorData: ErrorData, examples: FewShotExample[]): Promise<OpenAIResponse> {
    core.info('Falling back to GPT-3.5-turbo');
    
    // Remove screenshots for GPT-3.5 fallback
  ...
  private buildMessagesForO1(errorData: ErrorData, examples: FewShotExample[]): any[] {
    // o1 models don't support system messages, so combine everything into user message
    const systemPrompt = this.getSystemPrompt(false); // o1 doesn't support vision
    const userContent = this.buildUserContent(errorData, examples);
...
- Any visual indicators of what went wrong\n`
  ...
  private getSystemPrompt(hasScreenshots: boolean = false): string {
