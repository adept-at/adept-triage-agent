/**
 * Builds prompts for Cursor cloud agents that validate test fixes.
 *
 * The prompt tells the agent exactly what to do: check out the branch,
 * install deps, run the spec, and report structured results. It's
 * framework-aware (WDIO vs Cypress) so the agent knows the right commands.
 */

import { CursorValidationParams } from './cursor-cloud-validator';

export function buildValidationPrompt(params: CursorValidationParams): string {
...
function detectFramework(spec: string): string {
  if (spec.includes('.cy.') || spec.includes('cypress/')) return 'cypress';
  if (spec.includes('wdio') || spec.includes('test/specs/')) return 'webdriverio';
  return 'unknown';
}
...
function frameworkNotes(framework: string): string {
  switch (framework) {
    case 'webdriverio':
