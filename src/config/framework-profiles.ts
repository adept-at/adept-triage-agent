/**
 * Framework Profile Registry
 *
 * Single source of truth for per-framework behavior. Each concrete framework
 * maps to a {@link FrameworkProfile} carrying its human-readable label, the
 * command prefix used in prompts, and the fix-pattern block injected into the
 * fix-generation system prompt. `'unknown'` resolves to a NEUTRAL profile that
 * deliberately does NOT masquerade as Cypress.
 *
 * The `CYPRESS_PATTERNS` / `WDIO_PATTERNS` constants live here (moved out of
 * `agents/fix-generation-agent.ts`) so the registry owns them directly.
 */
...
/**
 * Neutral guidance for undetermined frameworks. Intentionally NOT Cypress —
 * routing an unattributed failure to `cy.*` patterns is the bug this registry
 * exists to fix. Tell the model to mirror whatever idioms the test file uses.
 */
...
const NEUTRAL_PROFILE: FrameworkProfile = {
  label: 'unknown',
  commandPrefix: '',
...
/**
 * Resolve the behavior profile for a framework. `'unknown'` (and any value
 * outside the concrete set) returns the NEUTRAL profile so callers never
 * accidentally inherit Cypress behavior for an unattributed failure.
 */
export function getFrameworkProfile(f: Framework): FrameworkProfile {
