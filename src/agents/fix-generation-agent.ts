...
/**
 * Structured causal trace that every fix must carry. See types.ts
 * FailureModeTrace for full documentation.
 */
...
export interface FixGenerationOutput {
  ...
  alternatives?: string[];
  /**
   * Required for v1.48+: structured causal trace the review agent uses to
   * verify the fix actually addresses the failure mode. Omission is a
   * CRITICAL issue at review time.
   */
...
> {
  ...
  /**
   * Get the system prompt, specialized by framework via the profile registry.
   *
   * For unknown/missing frameworks we emit a framework-NEUTRAL pattern block
   * (NOT Cypress) so an unattributed failure isn't pushed toward `cy.*` fixes.
   * We still warn once per agent instance so the gap stays visible in logs.
   */
  protected getSystemPrompt(framework?: Framework): string {
    const resolved: Framework = framework ?? 'unknown';
    if (resolved === 'unknown' && !this.warnedUnknownFramework) {
      this.warnedUnknownFramework = true;
      core.warning(
        `[FixGenerationAgent] Framework was unknown or missing (got ${JSON.stringify(framework)}); using framework-neutral fix guidance.`
      );
    }
    return COMMON_PREAMBLE + getFrameworkProfile(resolved).fixPatternBlock + COMMON_SUFFIX;
  }
  ...
  protected parseResponse(response: string): FixGenerationOutput | null {
    try {
      ...
      // Parse the optional-but-strongly-encouraged failureModeTrace. Keep
      // parsing lenient (accept partial traces, normalize to strings) — the
      // review agent is responsible for enforcing completeness.
      let failureModeTrace: FailureModeTraceOutput | undefined;
      if (parsed.failureModeTrace && typeof parsed.failureModeTrace === 'object') {
        ...
        failureModeTrace = {
          ...
          whyAssertionPassesNow:
            typeof t.whyAssertionPassesNow === 'string' ? t.whyAssertionPassesNow : '',
        };
      }
