 ...
 * the review prompt. Generous enough for a concrete multi-sentence trace,
 * tight enough that a runaway model can't blow up the review token budget.
 */
...
export interface ReviewInput {
  ...
  /**
   * Investigation output from the same run. Optional for backward
   * compatibility — but when present, the review agent audits the fix
   * against `recommendedApproach`, `primaryFinding`, `selectorsToUpdate`,
   * `verdictOverride`, and the full findings list. Without this, review is
   * blind to everything investigation concluded, which is a known
   * accuracy hole.
   */
...
- CRITICAL issues automatically mean rejection`;
  ...
  protected buildUserPrompt(input: ReviewInput, context: AgentContext): string {
    ...
    parts.push(
      ...
      `- **Suggested Approach (what analysis said the fix should do):** ${input.analysis.suggestedApproach}`,
    );
    ...
    if (patterns) {
      ...
      if (flaggedPatterns.length > 0) {
        parts.push(
          `- **Patterns flagged:** ${flaggedPatterns.join(', ')}`
        );
      }
    }
    ...
    // Investigation findings (R1). The review agent previously received only
    // analysis + proposedFix; without investigation it cannot audit whether
    // the fix addresses recommendedApproach, covers selectorsToUpdate, or
    // respects verdictOverride evidence. Those are exactly the checks that
    // determine fix correctness.
    if (input.investigation) {
      const inv = input.investigation;
      parts.push(
        '',
        '### Investigation Agent Findings (audit the fix AGAINST these)',
        `- **Investigation Confidence:** ${inv.confidence}%`,
        `- **Is Test Code Fixable:** ${inv.isTestCodeFixable}`,
        `- **Recommended Approach:** ${inv.recommendedApproach}`,
      );

      if (inv.primaryFinding) {
        parts.push(
          `- **Primary Finding:** ${inv.primaryFinding.description}`,
          `  - Severity: ${inv.primaryFinding.severity}`,
          `  - Relation to error: ${inv.primaryFinding.relationToError}`,
        );
      }

      if (inv.findings && inv.findings.length > 1) {
        // Show the full ranked findings list so the reviewer can see
        // secondary/tertiary signals, not just the primary one.
        parts.push('', '#### All Findings (ranked)');
        inv.findings.forEach((f, i) => {
          parts.push(
            `${i + 1}. [${f.severity}] ${f.type}: ${f.description}`,
            `   - Relation to error: ${f.relationToError}`
          );
        });
      }
      ...
      if (inv.selectorsToUpdate && inv.selectorsToUpdate.length > 0) {
        parts.push(
          '',
          '#### Selectors Investigation Said Need Updating',
          ...inv.selectorsToUpdate.map(
            (s) =>
              `- Current: \`${s.current}\` — ${s.reason}${s.suggestedReplacement ? ` → suggested: \`${s.suggestedReplacement}\`` : ''}`
          )
        );
      }
      ...
      if (inv.verdictOverride) {
        ...
        parts.push(
          '',
          `⚠️ **Verdict Override from Investigation:** ${o.suggestedLocation} (${o.confidence}% confidence)`,
          `Evidence: ${o.evidence.join('; ')}`,
          'The investigation agent concluded the failure location may differ from analysis. Audit whether the proposed fix is consistent with this.'
        );
      }
    }
    ...
    if (trace) {
      parts.push(
        '',
        '### Failure Mode Trace (MUST audit for quality)',
        `- **originalState:** ${formatTraceField(trace.originalState)}`,
        `- **rootMechanism:** ${formatTraceField(trace.rootMechanism)}`,
        `- **newStateAfterFix:** ${formatTraceField(trace.newStateAfterFix)}`,
        `- **whyAssertionPassesNow:** ${formatTraceField(trace.whyAssertionPassesNow)}`
      );
    } else {
      parts.push(
        '',
        '### Failure Mode Trace',
        '- **MISSING** — the fix did not provide a failureModeTrace. Per the system rules, flag this as CRITICAL and reject the fix.'
      );
    }
    ...
    if (context.productDiff && context.productDiff.files.length > 0) {
      parts.push(
        '',
        '### Product Repo Changes (MANDATORY review)',
        'Verify the proposed fix accounts for these product changes. If a product change caused the failure, the fix MUST adapt to the new product code.'
      );
