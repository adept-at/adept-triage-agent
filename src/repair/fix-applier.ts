...
export class GitHubFixApplier implements FixApplier {
  ...
  async applyFix(recommendation: FixRecommendation): Promise<ApplyResult> {
    ...
    try {
      ...
      // Pre-validate all changes before committing any (atomic approach)
      ...
      for (const change of recommendation.proposedChanges) {
        ...
        try {
          ...
          // Apply the change using position-aware replacement
          if (change.oldCode && change.newCode) {
            ...
            // Warn if the pattern appears multiple times
            ...
            if (secondMatch !== -1) {
              core.warning(
                `Pattern appears multiple times in ${filePath}. Replacing first occurrence at index ${matchIndex}. ` +
      ...
      // If no changes survived validation, clean up and fail
      if (pendingChanges.length === 0) {
        ...
        await this.cleanupBranch(
          branchName,
          'no valid changes after validation'
        );
        return {
          success: false,
          modifiedFiles: [],
      ...
      // For multi-file fixes, abort if some changes failed validation
      // to avoid leaving the branch in a semantically incomplete state
      ...
      // All validated — now commit changes
      for (const pending of pendingChanges) {
        try {
          const commitMessage = `fix(test): ${pending.justification.slice(
            0,
            50
          )}
...
Confidence: ${recommendation.confidence}%`;
          ...
          if (commitSha) {
            ...
            core.warning(
              `No commit SHA returned for ${pending.filePath}, using previous value`
            );
          }
          ...
          const errorMsg = getErrorMessage(
            commitError,
            `committing ${pending.filePath}`
          );
          core.error(errorMsg);

          // Commit-phase failure: clean up branch to avoid partial state
          await this.cleanupBranch(
            branchName,
            `commit failed for ${pending.filePath}`
          );
          return {
            success: false,
            modifiedFiles,
